...
class RequestApi {
    ...
    func requestBeers(onComplete : @escaping ([Beer]?) -> Void){
        ...
        GetApi.session.dataTask(with: url) { (data: Data?, response: URLResponse?, error: Error?) in
            if error != nil {
                onComplete(nil)
            }else{
...
func requestPicture(url: String, onComplete : @escaping (UIImage?) -> Void){
    ...
    GetApi.session.downloadTask(with: url, completionHandler: { (url: URL?, response: URLResponse?, error: Error?) in
